...
interface TestimonialProps {
  heading?: string;
  testimonials?: TestimonialPropItem[];
}
...
const DEFAULT_HEADING = 'Client words';
const FALLBACK_AVATAR = '/images/avatar/av4.jpg';

const Testimonial = ({ heading = DEFAULT_HEADING, testimonials }: TestimonialProps) => {
  const sourceTestimonials: (TestimonialCard | TestimonialPropItem)[] = testimonials ?? testimonialCards;

  const normalizedTestimonials = sourceTestimonials.map<TestimonialCard>((item, index) => ({
    id: item.id ?? index + 1,
    name: item.name,
